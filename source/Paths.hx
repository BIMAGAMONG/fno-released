...
class Paths{
	...
	static public function getLevelThing(path:String):String return "assets/minigames/" + path;
	...
	inline static public function codeShitVideo(key:String) return 'assets/codeshit/$key.mp4';
	inline static public function codeShitImage(key:String) return 'assets/codeshit/$key.png';
	inline static public function json(key:String, ?type:String = '') return 'songs:assets/songs/$key' + type + '.json';
	inline static public function music(key:String, ?library:String) return getPath('music/$key.$SOUND_EXT', MUSIC, library);
	inline static public function voices(song:String, ?type:String = ''){
		song = StringTools.replace(song," ", "-");
		return 'songs:assets/songs/${song.toLowerCase()}/Voices' + type + '.$SOUND_EXT';
	}
	...
	inline static public function getSparrowAtlas(key:String, ?library:String) return FlxAtlasFrames.fromSparrow(image(key, library), file('images/$key.xml', library));
	...
	inline static public function bg(key:String) return 'assets/bg/$key.png';
	inline static public function bgSheet(key:String) return FlxAtlasFrames.fromSparrow('assets/bg/$key.png', 'assets/bg/$key.xml');
	inline static public function inst(song:String, ?type:String = ''){
		song = StringTools.replace(song," ", "-");
		return 'songs:assets/songs/${song.toLowerCase()}/Inst' + type + '.$SOUND_EXT';
	}
}