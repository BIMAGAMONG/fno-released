...
class ChartingState extends MusicBeatState{
	...
	var noteStyle:Int = 0;
	var styles:Array<String> = ['normal', 'gfNote', 'playerfour', 'normAndFourDuet', 'normAndgfNote'];
	var noteTypeTxt:FlxText = new FlxText(0,100,0,'Charting: ', 16);
	var txt:FlxText = new FlxText(0, 0, 0, '');
	var txtVar:FlxTypedGroup<FlxText> = new FlxTypedGroup<FlxText>();

	override function create(){
		curSection = lastSection;

		if (PlayState.SONG != null) _song = PlayState.SONG;
		else _song = {song: 'Test', notes: [], bpm: 150, needsVoices: true, player1: 'bf', player2: 'dad', gfVersion: 'gf', noteStyle: 'normal', stage: 'stage', speed: 1};
		...
		blackBorder.antialiasing = false;
		...
		noteTypeTxt.scrollFactor.set(0,0);
		add(noteTypeTxt);
		updateHeads();
		add(txtVar);
		super.create();
	...
	function addSongUI():Void{
		...
		waveformUseInstrumental = new FlxUICheckBox(150, 130, null, null, "Waveform for Instrumental", 100);
		...
		var check_mute_voc = new FlxUICheckBox(10, 220, null, null, "Mute Vocals (in editor)", 100);
		check_mute_voc.checked = false;
		check_mute_voc.callback = function(){
			var vol:Float = 1;
			if (check_mute_voc.checked) vol = 0;
			vocals.volume = vol;
		};

		var saveButton:FlxButton = new FlxButton(110, 8, "Save", function(){saveLevel();});
		var reloadSong:FlxButton = new FlxButton(saveButton.x + saveButton.width + 10, saveButton.y, "Reload Audio", function(){loadSong(_song.song);});
		...
		var restart = new FlxButton(10,140,"Reset Chart", function(){
            for (ii in 0..._song.notes.length) for (i in 0..._song.notes[ii].sectionNotes.length) _song.notes[ii].sectionNotes = [];
            resetSection(true);
        });
		...
		hitsounds.callback = function(){playClaps = hitsounds.checked;};
		...
		var shiftNoteButton:FlxButton = new FlxButton(10, 335, "Shift", function(){shiftNotes(Std.int(stepperShiftNoteDial.value),Std.int(stepperShiftNoteDialstep.value),Std.int(stepperShiftNoteDialms.value));});
		...
		var player1DropDown = new FlxUIDropDownMenu(10, 100, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String){_song.player1 = characters[Std.parseInt(character)];});
		player1DropDown.selectedLabel = _song.player1;

		var player1Label = new FlxText(10,80,64,'Player 1');
		var player2DropDown = new FlxUIDropDownMenu(140, 100, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String){_song.player2 = characters[Std.parseInt(character)];});
		player2DropDown.selectedLabel = _song.player2;

		var player2Label = new FlxText(140,80,64,'Player 2');
		var gfVersionDropDown = new FlxUIDropDownMenu(10, 200, FlxUIDropDownMenu.makeStrIdLabelArray(gfVersions, true), function(gfVersion:String){_song.gfVersion = gfVersions[Std.parseInt(gfVersion)];});
		gfVersionDropDown.selectedLabel = _song.gfVersion;

		var gfVersionLabel = new FlxText(10,180,64,'Girlfriend');
		var stageDropDown = new FlxUIDropDownMenu(140, 200, FlxUIDropDownMenu.makeStrIdLabelArray(stages, true), function(stage:String){_song.stage = stages[Std.parseInt(stage)];});
		stageDropDown.selectedLabel = _song.stage;
		
		var stageLabel = new FlxText(140,180,64,'Stage');
		var noteStyleDropDown = new FlxUIDropDownMenu(10, 300, FlxUIDropDownMenu.makeStrIdLabelArray(noteStyles, true), function(noteStyle:String){_song.noteStyle = noteStyles[Std.parseInt(noteStyle)];});
	...
	function addSectionUI():Void{
		...
		var copyButton:FlxButton = new FlxButton(10, 130, "Copy last section", function(){copySection(Std.int(stepperCopy.value));});
	...
	function loadSong(daSong:String):Void{
		...
		FlxG.sound.playMusic(Paths.inst(daSong, PlayState.type), 0.6);
		vocals = new FlxSound().loadEmbedded(Paths.voices(daSong, PlayState.type));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>){
		if (id == FlxUICheckBox.CLICK_EVENT){
			...
			switch (label){
				case 'Camera Points to P1?': _song.notes[curSection].mustHitSection = check.checked;
				case 'Change BPM': 
					_song.notes[curSection].changeBPM = check.checked;
					updateGrid();
					FlxG.log.add('changed bpm shit');
				case "Alternate Animation": _song.notes[curSection].altAnim = check.checked;
			}
			...
			if (wname == 'section_length'){
				...
				if (nums.value <= 0.1) nums.value = 0.1;
	...
	override function update(elapsed:Float){
		noteTypeTxt.text = 'X & Z (+ hold SHIFT)\nto switch note Type\nCharting: ' + styles[noteStyle];
		snapText.text = "Snap: 1/" + snap + " (" + (doSnapShit ? "Control to disable" : "Snap Disabled, Control to renable") + ")\nAdd Notes: 1-8 (or click)\n";
		curStep = recalculateSteps();
		if (FlxG.keys.justPressed.CONTROL) doSnapShit = !doSnapShit;
		...
		if (FlxG.mouse.justPressed){
			...
			}else if (FlxG.mouse.x > gridBG.x && FlxG.mouse.x < gridBG.x + gridBG.width && FlxG.mouse.y > gridBG.y && FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps)) addNote();
		}
		...
		if (FlxG.mouse.x > gridBG.x && FlxG.mouse.x < gridBG.x + gridBG.width && FlxG.mouse.y > gridBG.y && FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps)){
			...
			else dummyArrow.y = Math.floor(FlxG.mouse.y / GRID_SIZE) * GRID_SIZE;
		}
		...
		if (!typingShit.hasFocus){
			...
			if (FlxG.keys.justPressed.X && FlxG.keys.pressed.SHIFT){
				this.noteStyle ++;
				if (noteStyle > styles.length - 1) noteStyle = 0;
			}
			if (FlxG.keys.justPressed.Z && FlxG.keys.pressed.SHIFT){
				this.noteStyle --;
				if (noteStyle < 0) noteStyle = styles.length - 1;
			}
			...
			if (FlxG.mouse.wheel != 0){
				...
				if (doSnapShit) FlxG.sound.music.time = stepMs - (FlxG.mouse.wheel * Conductor.stepCrochet / snap);
				else FlxG.sound.music.time -= (FlxG.mouse.wheel * Conductor.stepCrochet * 0.4);
	...
	function recalculateSteps():Int{
		...
		for (i in 0...Conductor.bpmChangeMap.length) if (FlxG.sound.music.time > Conductor.bpmChangeMap[i].songTime) lastChange = Conductor.bpmChangeMap[i];
	...
	function updateNoteUI():Void{if (curSelectedNote != null) stepperSusLength.value = curSelectedNote[2];}
	function updateGrid():Void{
		for (i in txtVar.members){
			if (i != null && txtVar.members.length != 0){
				i.destroy();
				txtVar.remove(i);
			}
		}
		...
		for (i in sectionInfo){
			...
			if (curSelectedNote != null) if (curSelectedNote[0] == note.strumTime) lastNote = note;

			txt = new FlxText(note.x, note.y, 0, daStyle + '');
			txt.size = 10;
			txt.borderSize = 2;
	...
	private function addSection(lengthInSteps:Int = 16):Void{
		var sec:SwagSection = {lengthInSteps: lengthInSteps, bpm: _song.bpm, changeBPM: false, mustHitSection: true, sectionNotes: [], typeOfSection: 0, altAnim: false};
	...
	function selectNote(note:Note):Void{
		...
		for (i in _song.notes[curSection].sectionNotes){
			if (i.strumTime == note.strumTime && i.noteData % 4 == note.noteData) curSelectedNote = _song.notes[curSection].sectionNotes[swagNum];
			swagNum += 1;
		}
		updateGrid();
		updateNoteUI();
	...
	private function newSection(lengthInSteps:Int = 16,mustHitSection:Bool = false,altAnim:Bool = true):SwagSection{
		var sec:SwagSection = {lengthInSteps: lengthInSteps, bpm: _song.bpm, changeBPM: false, mustHitSection: mustHitSection, sectionNotes: [], typeOfSection: 0, altAnim: altAnim};
		return sec;
	}
	...
	function shiftNotes(measure:Int=0,step:Int=0,ms:Int = 0):Void{
		...
		if(millisecadd > 0)	for(i in 0...totaladdsection) newSong.unshift(newSection());
		for (daSection1 in 0..._song.notes.length) newSong.push(newSection(16,_song.notes[daSection1].mustHitSection,_song.notes[daSection1].altAnim));
	...
	private function addNote(?n:Note):Void{
		...
		if (n != null) _song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.noteStyle]);
		else _song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteStyle]);
	...
	function getStrumTime(yPos:Float):Float {return FlxMath.remapToRange(yPos, gridBG.y, gridBG.y + gridBG.height, 0, 16 * Conductor.stepCrochet);}
	function getYfromStrum(strumTime:Float):Float {return FlxMath.remapToRange(strumTime, 0, 16 * Conductor.stepCrochet, gridBG.y, gridBG.y + gridBG.height);}
	...
	function getSectionBeats(?section:Null<Int> = null){
		...
		if(_song.notes[section] != null) val = _song.notes[section].lengthInSteps /4;
