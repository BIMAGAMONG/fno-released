class EtternaFunctions{
    ...
    public static function getMapMaxScore():Int{return (getNotes() * 350);}
    public static function wife3(maxms:Float, ts:Float){
        ...
        if (maxms <= ridic)  return max_points;
        else if (maxms <= zero) return max_points * erf((zero - maxms) / dev);
        else if (maxms <= max_boo_weight) return (maxms - zero) * miss_weight / (max_boo_weight - zero);
