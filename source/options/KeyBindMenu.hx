...
class KeyBindMenu extends FlxSubState{
    ...
    var keyText:Array<String> = LangUtil.getKeybindNames();
    var defaultKeys:Array<String> = ["A", "S", "W", "D", "R"];
    var curSelected:Int = 0;
    var keys:Array<String> = [FlxG.save.data.leftBind, FlxG.save.data.downBind, FlxG.save.data.upBind,FlxG.save.data.rightBind];
    var tempKey:String = "";
	...
	override function create(){	
		...
		keyTextDisplay.setFormat(Paths.font("LilitaOne-Regular.ttf"), 42, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        ...
        infoText = new FlxText(-10, 580, 1280, '${LangUtil.translate("ENTER")} - ${LangUtil.translate("select")}\n${LangUtil.translate("ESC")} - ${LangUtil.translate("save and exit")}\n${LangUtil.translate("BACKSPACE")} - ${LangUtil.translate("reset and exit")}', 72);
        ...
        FlxTween.tween(infoText, {alpha: 1}, 1.4, {ease: FlxEase.expoInOut, onComplete: function(twn:FlxTween){block = false;}});
        ...
        if (!block){
            switch(state){
                case "select":
                    ...
                    if (FlxG.keys.justPressed.ENTER) state = "input";
                    else if (FlxG.keys.justPressed.ESCAPE) quit();
                    else if (FlxG.keys.justPressed.BACKSPACE) reset();
                case "input":
    ...
    function reset(){
        for(i in 0...5) keys[i] = defaultKeys[i];
        quit();
    }
	...
	function addKey(r:String){
        ...
        for(x in 0...keys.length) {
            var oK = keys[x];
            if(oK == r) keys[x] = null;
            if (notAllowed.contains(oK)) return;
        }
